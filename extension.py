...
def main():
    ...
    axs1[0, 0].set_title('Guassian filter')
    axs1[0, 0].imshow(px_array, cmap='gray')

    # Stretch the image from 0 to 255.
    print("Computing 0 to 255 stretched image...")
    px_array = scaleTo0And255AndQuantize(px_array, image_width, image_height)
    axs1[0, 1].set_title('Contrast stretched')
    ...
    px_array = cv.morphologyEx(np.array(px_array, dtype=np.uint8), cv.MORPH_CLOSE, kernel, iterations=5)
    axs1[1, 0].set_title('Morphological close')
    axs1[1, 0].imshow(px_array, cmap='gray')

    # Compute connected components image.
    print("Computing connected components...")
    (px_array, components) = computeConnectedComponentLabeling(
        px_array, image_width, image_height)
